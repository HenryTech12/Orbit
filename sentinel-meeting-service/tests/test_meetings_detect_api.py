...
from app.models import MeetingDetection


def test_detect_endpoint_with_zoom_link_and_time(client):
    resp = client.post("/meeting-links/detect", json={
    ...
    assert body["platform_meeting_id"] == "1234567890"
    assert body["detected_datetime"] is not None


def test_detect_endpoint_with_teams_link_and_time(client):
    resp = client.post("/meeting-links/detect", json={
        "message": "Wadhwani session today at 3pm https://teams.microsoft.com/l/meetup-join/19%3ameeting_abc",
...
def test_detect_endpoint_without_link_rejected(client):
    resp = client.post("/meeting-links/detect", json={"message": "no link here at all"})
...
def test_list_detections(client):
    client.post("/meeting-links/detect", json={
        "message": "Call today at 9am https://zoom.us/j/1",
        "reference_time": "2026-09-17T08:00:00",
    })
    client.post("/meeting-links/detect", json={
        "message": "Call today at 10am https://teams.microsoft.com/meet/2",
        "reference_time": "2026-09-17T08:00:00",
    })

    resp = client.get("/meeting-links/detections")
...
def test_reminder_scheduled_at_correct_offset(db_session):
    detection = MeetingDetection(
        raw_message="msg",
        platform="teams",
        meeting_url="https://teams.microsoft.com/meet/1",
...
def test_reminder_not_scheduled_without_detected_datetime(db_session):
    detection = MeetingDetection(raw_message="msg", platform="zoom", meeting_url="https://zoom.us/j/1", detected_datetime=None)
    db_session.add(detection)
    db_session.commit()
    db_session.refresh(detection)
...
def test_due_reminders_filters_correctly(db_session):
    detection = MeetingDetection(
        raw_message="msg", platform="zoom", meeting_url="https://zoom.us/j/1",
        detected_datetime=datetime(2026, 9, 18, 19, 0, 0),
    )
    db_session.add(detection)
    db_session.commit()
...
def test_reminders_due_endpoint_and_mark_sent(client):
    detect_resp = client.post("/meeting-links/detect", json={
    ...
    due_resp = client.get("/meeting-links/reminders/due")
    assert due_resp.status_code == 200
    due_list = due_resp.json()
    assert len(due_list) == 1
    assert due_list[0]["sent"] is False

    mark_resp = client.post(f"/meeting-links/reminders/{due_list[0]['id']}/sent")
    assert mark_resp.status_code == 200
    assert mark_resp.json()["sent"] is True

    due_resp_after = client.get("/meeting-links/reminders/due")
    assert due_resp_after.json() == []


def test_mark_sent_nonexistent_reminder(client):
    resp = client.post("/meeting-links/reminders/does-not-exist/sent")
    assert resp.status_code == 404


def test_webhook_creates_and_processes_zoom_meeting_automatically(client):
    resp = client.post("/meeting-links/webhook", json={
        "platform": "zoom",
...
def test_webhook_creates_and_processes_teams_meeting_automatically(client):
    resp = client.post("/meeting-links/webhook", json={
        "platform": "teams",
        "platform_meeting_id": "419860837373470",
        "topic": "Wadhwani Session",
    })
    assert resp.status_code == 201
    body = resp.json()
    assert body["title"] == "Wadhwani Session"
    assert body["platform"] == "teams"
    assert body["status"] == "completed"
    assert body["recording_url"]
    assert len(body["chunks"]) >= 1
...
def test_webhook_rejects_unsupported_platform(client):
    resp = client.post("/meeting-links/webhook", json={
...
def test_webhook_links_back_to_originating_detection(client):
    detect_resp = client.post("/meeting-links/detect", json={
        "message": "Call tomorrow at 7pm https://teams.microsoft.com/meet/1234567890",
        "reference_time": "2026-09-17T08:00:00",
    })
    detection_id = detect_resp.json()["id"]

    webhook_resp = client.post("/meeting-links/webhook", json={
        "platform": "teams",
        "platform_meeting_id": "1234567890",
    ...
    detections = client.get("/meeting-links/detections").json()
