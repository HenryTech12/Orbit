...
export class BaileysGateway implements WhatsAppGateway {
  ...
  // ─────────────────────────────────────────────────────────────
  // sendMessage — now supports native WhatsApp mentions.
  // The `mentions` array must contain JIDs like
  // "250783188655@s.whatsapp.net". Any @<digits> in the text
  // that matches a JID in this array is rendered as a clickable
  // blue tag by WhatsApp's client.
  // ─────────────────────────────────────────────────────────────
  async sendMessage(
    chatId: string,
    text: string,
    ...
    console.log(
      ...
      }${mentions && mentions.length > 0 ? ` [${mentions.length} mention(s)]` : ''}`,
    );
  }
  // ─────────────────────────────────────────────────────────────
  // sendDocument — send a stored file (PDF, DOCX) to a chat.
  // ─────────────────────────────────────────────────────────────
  async sendDocument(
    chatId: string,
    filePath: string,
    ...
    console.log(
      `WhatsApp document sent → ${chatId} (${fileName}, ${buffer.length} bytes)${
        replyToMessageKey ? ` (reply to ${replyToMessageKey})` : ''
      }`,
    );
  }
